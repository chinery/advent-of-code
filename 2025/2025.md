...
**update day 7**
as of day 7, unsure if it made any difference – the race for the leaderboard means there's no chill at the top, and the people who got stuck on day 1/2 quickly dropped off – hopefully we can encourage people to pick it back up

**update day >12**
confirmed that the rate of one-a-day still meant most people ended up overwhelmed and falling off – only two of us completed it (as of 31st Dec)

### Challenge
I decided to do each day of this year's puzzles in a different programming language.

At the time I knew it was a silly idea, it's not like I'm a polyglot. But I wanted to challenge myself, and there was no one specific “new” language I wanted to commit to for the full thing. And 12 languages is much more manageable than 25.
...
9. [Mojo](#day-9--mojo)
10. [Swift](#day-10--swift)
11. [Ruby+Sorbet](#day-11--rubysorbet)
12. [Julia](#day-12--julia)

Languages I considered but didn't end up using
- C++ (which I've used before)
- Gleam (which I haven't)

originally this part then said 
> _“if I see some sort of backtracking constraint satisfaction problem, maybe I'll try Prolog? (I want to understand it better, but I suspect it's not the right venue.)”_
...
I actually found day_1_2 one of the most fiddly day 1 problems so far, but maybe being on the call didn't help. Handling the edge case of what happens when the dial lands on zero (and not double counting it) definitely hurt my head a bit.
...
#### Day 2 – Javascript/Typescript/Node.js
...
for i in range(121000, 343000+1):
    if test(str(i)) # ...
```
and then you think “how do I make `test` as efficient as possible?”. And in that case, a regex is a really neat solution e.g. `^(\d)+\1+$` (Almost so powerful it's cheating! And worth pointing out that this language is [not regular](https://en.wikipedia.org/wiki/Regular_language) – being able to do backreferences makes modern regex engines really quite expressive.)

But this is what I'm calling "brute-force" – you don't need to iterate through every possible number and test it, because you can describe every number that meets the criteria. 
...
Getting things running in VS Code on macOS was fine, all things considered (no need for mono these days), and once it was working, everything was working (debugging etc). I like the new (to me) support for more "script-like" files that have top level statements, which simplifies the entry point – minor but especially nice for AoC.
...
Well, it was great. I like all the options for result unpacking – from "just do it" to "let's handle this with an error message" to "okay this should probably be handled properly" (which I don't think I actually needed for this toy puzzle).
...
Rust is meant to be fast so I had to time it. `94.5µs` for part two on my M1 MacBook, if I switch off low power mode and put it in release profile. Seems fast enough I guess!

#### Day 6 – J

In my first chat with ChatGPT, it said: 
> #### APL or J
> - If you want to spend an evening feeling like you’ve joined a secret cult.
> - Ridiculous power density for array puzzles.
> - Harder to “stay idiomatic” without understanding the language deeply, but incredibly fun.

When I told a colleague about the challenge, he asked if I was going to use an array language like APL. I've used a lot of Matlab and NumPy, so array/vector thinking isn't new to me. But J is the first “terse” language I've tried – I've seen those wild golf solutions before, but never tried to understand them. I didn't actually know there was a link between the two. 
...
The puzzle itself was quite unremarkable – just don't fall into the trap of feeling like you've got to keep track of anything you don't actually have to! 
...
I did find it really amusing that the built-in package manager required me to go and install dependencies myself before it would install the statistics package. (Well, dependency – that could have gotten boring fast.)
...
GPU support was a headline feature, and I thought that might be fun...but it just wasn't that sort of puzzle. I considered it for part one, but after looking into it more, it turns out it still requires the all the usual ceremony to set up memory buffers etc. I'd hoped it might be a bit more ergonomic!

`.🔥` as a file extension is just funny for some reason. And [pixi](https://pixi.sh/latest/) seems like a good package manager.
...
I realised the problem could be reduced to simultaneous equations, and spent some time looking at linear algebra options in Swift, and even got a bunch of LAPACK examples running. But this wasn't the right idea – a little knowledge is dangerous. I could find solutions to the linear system, but they ended up being non-integer, and/or non-minimal. So I turned to a simplified Gaussian elimination process. But...of course...most of the inputs are under-constrained. I had depth first search implemented for once I'd exhausted the row reductions...buuuut it was clear that the code would never finish running. 
...
With a little more retrospect – I think my Gaussian elimination is not as robust as it should be, as I was needlessly avoiding creating equations that did not make sense in context (e.g. I did not allow `a - b = ...`) but they would have still solved correctly if I'd done it right. The search space could have been reduced a lot more if I just forced lower triangular zeroes as you would “typically” do. It's been too long since I did the maths and I didn't revise it properly. At this point (31st Dec) it seems unlikely I'll actually go back and change the code but a lesson for the future!

#### Day 11 – Ruby+Sorbet
(Solved on 12th Dec, this text written on 31st) I've been lax in updating this doc timeously, I'll try to remember the details.

Day 11's puzzle did not give me as much trouble as day 10's, but still ended up taking a larger chunk out of my day off than I expected!

I could sense that part one would be easy to brute force, and it was worth getting to part two to see what we were dealing with. I worked out the graph was acyclic, and counting the paths from `svr -> fft` could be done backwards, and `dac -> out` could be done forwards. So I just needed `fft -> dac`.

Unlike day 10, I liked how in this puzzle it was obvious that neither forwards nor backwards was going to work as soon as I hit run. So I worked on an idea that started at both ends and met in the middle. I didn't like how many times I messed up and had my pointers skip past each other! I've never hit such long timeouts on the advent of code submission page. 

![Massive graph](11/graphviz.svg)

After looking at the graph of nodes I realised I could label each node with its maximum distance from the root, and that meant I could stop pointers from moving “too fast”. Hats off to graphviz (in particular the dot engine and [this website](https://dreampuf.github.io/GraphvizOnline/?engine=dot)) for being able to handle the massive graph without blinking.

Ruby was fun though. It's odd to try to pick up older languages where there's already lots of opinions – like you're expected to already know certain things. I mean, for a start, it was weird to install a language extension into VS Code that's owned by Shopify. But that aside, it suggested using Sorbet for type checking, so I thought why not? 

I'm not going to say it's a hard requirement, but I've realised I vastly prefer languages where the IDE can help you write correct code. On day two I had to switch from Javascript to Typescript because I want to be able to write `if (myStr.` and have the autocomplete tell me the valid methods. Is it `.include` or `.contains` or `.indexOf` or...? I don't want to have to search for this.

So having types makes development a significantly nicer experience. Of course being new to Ruby, I've never used it without Sorbet. So it felt like jumping through hoops at time but that's just the cruft of an older language.

Unfortunately I'm writing this too late to remember more detailed thoughts. Bits of syntax are fun, like `x unless y`, and I sort of like the ability to call functions without parentheses in theory, but then it turns out the style guides suggest using them almost always anyway.

#### Day 12 – Julia
I'd been wanting to try Julia since ChatGPT referred to it as "like Python if NumPy had eaten the rest of the language".

It was during prep for day 12 that I felt like I was starting to see how each programming language is just a different set of decisions along many axes. Want your scopes denoted by braces, end statements, whitespace? Want infix or prefix notation for methods? Want some fun GC-free movement feature?

Julia has some features of languages I'd used earlier in the experiment that I really like. For example, I like having `!` versions of functions to indicate that they modify arguments, like Ruby. I like how arrays are first-class, and the powerful broadcasting feature, echoing (/surpassing) MATLAB or NumPy. And the promise of "vectorise or loop, both will be fast" is very appealing.

I did not have “index from 1” on my bingo card for axes in which languages would differ! Honestly can't say that's something I'm typically looking for. (Though [technically it's optional](https://docs.julialang.org/en/v1/devdocs/offset-arrays/), which seems equally very powerful and very confusing...)

I really liked UFCS in [Nim](#day-7-nim) (when it worked the way I expected), and I struggled with discovering functions in Julia simply because there was no way for the IDE to know I wanted to do something to a particular argument. But maybe I have too strong of an object-oriented mindset – I even wrote ‘object’ instead of ‘argument’ on my first pass in that last sentence. Certainly this has been [debated before](https://github.com/JuliaLang/julia/issues/31779). 

Maybe there's a better way to get autocomplete. And maybe I would have found it, had day 12's puzzle not turned out to be a...joke? A troll? A gift? I'm not sure which. This is only my second time completing advent of code so maybe this is a tradition I didn't know. I've seen many puzzles where your full input ends up making the puzzle much harder than it seems from the description. I'd not seen one where it looks so hard that you are supposed to think “this doesn't look possible, maybe every input is trivially solvable”.

So day 12 lived rent-free in my head for about a week – I was daunted, and had been trying to come up with any sort of viable strategy. A few ideas but nothing simple. I discussed it with some colleagues, and they agreed that the task looked near impossible unless we were missing something. We'd pointed out that obviously you could rule out certain trivial combinations, “but surely the input won't contain any of those” I said. One of them looked up a solution...and basically gave away the fact that, no, it's all trivial.

So I didn't really get to exercise Julia after all! Maybe one to come back to one day.

### Conclusions

*note from 12th April 2026* – I've just come back to find some uncommitted notes, and this section blank. I guess I never got around to writing any conclusions!

Now several months removed, I think back on it mostly fondly. As is not entirely new, the puzzles took a bigger impact on my day-to-day life than expected. A reminder that something can feel like it'll be quick because the path forwards seems obvious, but you shouldn't discount how long it'll take to actually *do* the thing. The urgency of the leaderboard might have pushed me through with ADHD-fuelled fervour, but that's not necessary healthy for a hobby!

I think my self-inflicted challenge confirms that the understanding of algorithms and data structures (or lack of understanding) is more important than knowledge of any particular language. Certainly when it comes to syntax. But even the native quirks/pitfalls of each language are more accessible than ever, thanks to well written guides and the customised support of LLMs.
