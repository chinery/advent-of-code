...
# yes I resorted to a global variable, so sue me
STOP = False


class Character:
    def __init__(self, faction, attack):
        self.hp = 200
        self.attack = attack
    ...
    def __init__(self, text, attack):
        ...
        for i in range(0, len(self.grid)):
            for j in range(0, len(self.grid[0])):
                if self.grid[i][j] == 'G':
                    self.characters.append([i, j, Character('G', 3)])
                elif self.grid[i][j] == 'E':
                    self.characters.append([i, j, Character('E', attack)])
...
if __name__ == "__main__":
    ...
    # originally started at 4, turns out 14 is magic number
    attack = 14
    while True:
        print("Attack value {}".format(attack))
        game = Game(lines, attack)
        outcome = game.play()
        if outcome != -1:
            print("Elves win! Outcome: {}".format(outcome))
            break
        print("An elf died.")
        STOP = False
        attack += 1
